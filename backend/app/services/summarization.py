...
class SummarizationService:
    ...
    async def check_and_summarize(self, db: AsyncSession, story_id: str) -> Summary | None:
        """Check if conditions are met and generate summaries as needed."""
        ...
        expected_small = archived_count // 10

        if small_count < expected_small:
            return await self._generate_small_summary(db, story_id, small_count + 1)
        ...
        if small_count >= 3 and large_count < (small_count // 3) + 1:
            return await self._update_large_summary(db, story_id)

        return None

    async def _generate_small_summary(self, db: AsyncSession, story_id: str, level: int) -> Summary | None:
        start_chapter = (level - 1) * 10 + 1
        end_chapter = level * 10

...
export function WorldBookList({
  ...
  return (
    <div className="space-y-1.5">
      <div className="mb-2">
        <input
          className="w-full bg-transparent text-sm font-semibold border-b border-transparent hover:border-gray-300 focus:border-blue-500 outline-none px-1 py-0.5 text-gray-700"
          value={worldBookName}
          placeholder="世界书名称"
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-1 px-2 py-1.5 text-xs border border-dashed border-gray-300 rounded-lg hover:border-blue-500 hover:text-blue-500 text-gray-400 mb-2"
      >
        <Plus size={14} /> 新增条目
      </button>
      ...
      {sorted.map((entry, idx, arr) => (
        ...
        >
          <div className="p-2 flex items-center gap-1.5">
            ...
            <span className="font-medium truncate flex-1 text-xs text-gray-700">{entry.name}</span>
            ...
            <button
              ...
              className={`p-0.5 rounded hover:bg-gray-200 ${
                entry.status === 'active' ? 'text-green-500' : 'text-gray-300 hover:text-gray-500'
              }`}
          ...
          <div className="border-t border-gray-100 px-3 py-1.5">
            ...
            {entry.aliases?.length > 0 && (
              <p className="text-xs text-gray-400 mt-1">别名: {entry.aliases.join(', ')}</p>
            )}
      ...
      {entries.length === 0 && (
        <p className="text-gray-400 text-sm p-3 text-center">暂无世界书条目</p>
      )}
      ...
      <div className="flex gap-1 mt-3 pt-3 border-t border-gray-100">
        <select
          ...
          className="bg-gray-100 border border-gray-200 rounded px-1 py-1 text-xs w-16"
        >
        ...
        <button
          ...
          className="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 border border-gray-200 rounded hover:bg-gray-200 text-gray-600"
        >
          <Download size={12} /> 导出
        </button>
        <button
